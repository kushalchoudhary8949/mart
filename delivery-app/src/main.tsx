...
function App() {
  ...
  useEffect(() => { if (!token || !navigator.geolocation) return; const timer = window.setInterval(() => navigator.geolocation.getCurrentPosition((p) => mine.filter((o) => o.status !== 'DELIVERED').forEach((o) => request(`/delivery/orders/${o.id}/location`, { method: 'PATCH', body: JSON.stringify({ latitude: p.coords.latitude, longitude: p.coords.longitude }) }).catch(() => undefined))), 10000); return () => clearInterval(timer); }, [token, mine]);
