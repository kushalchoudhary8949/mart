...
export function listAvailableOrders() {
  return prisma.order.findMany({ where: { status: OrderStatus.READY_FOR_PICKUP, deliveryPartnerId: null }, include: orderInclude, orderBy: { readyForPickupAt: 'asc' } });
}
...
export function listPartnerOrders(partnerId: number, completed = false) {
  return prisma.order.findMany({
    where: completed
      ...
      : { deliveryPartnerId: partnerId, status: { in: [OrderStatus.READY_FOR_PICKUP, OrderStatus.OUT_FOR_DELIVERY] } },
