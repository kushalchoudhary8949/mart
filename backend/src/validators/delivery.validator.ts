...
export const partnerSchema = z.object({ userId: id, phone: z.string().trim().min(8).max(20), name: z.string().trim().min(2).max(100), vehicleType: z.string().trim().min(2).max(50), vehicleNumber: z.string().trim().min(3).max(30), profileImage: z.string().url().optional(), isAvailable: z.boolean().optional() });
